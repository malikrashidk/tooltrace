...
router.post("/notes", authMiddleware, emailVerificationMiddleware, async (req, res) => {
  try {
    ...
    res.status(400).json({ error: _error.message || "Failed to create note" });
  }
});
...
router.patch("/notes/:id", authMiddleware, emailVerificationMiddleware, async (req, res) => {
  try {
    ...
    res.status(400).json({ error: _error.message || "Failed to update note" });
  }
});
...
router.get("/receipts", authMiddleware, paidPlanMiddleware, async (req, res) => {
  try {
    ...
    console.error("Fetch receipts error:", _error);
  ...
  try {
    ...
    if (process.env.R2_BUCKET_NAME) {
      try {
        ...
        console.error("R2 Upload failed:", _e);
        ...
        return res.status(500).json({ error: "R2 Upload Failed: " + (_e as Error).message });
      }
    }
    ...
    console.error("Receipt upload error:", _error);
    if (_error.code === '23502' || _error.message?.includes('null value')) {
      return res.status(400).json({
        error: `Missing required field.Please ensure all required fields are provided.`
      });
    }
    res.status(500).json({ error: _error.message || "Failed to upload receipt" });
  }
});
  ...
  try {
    ...
    if (!receipt.fileUrl.startsWith("data:") && !receipt.fileUrl.startsWith("http")) {
      try {
        ...
        console.error("Failed to delete from R2:", _e);
      }
    }
...
router.post("/tools/match", authMiddleware, async (req, res) => {
  try {
    ...
    console.error("Tool match error:", _error);
